		...
		booksById: function(id) {
			...
			db.transaction(function(tx) {
				tx.executeSql('SELECT Isbn FROM BOOKS WHERE id = ?', [id], function(tx, results) {
					...
					var arr = [];
					for(var i = 0; i < r.length; i++) {
						tx.executeSql('SELECT s.*, b.Discount, b.IdCustomer FROM STORE s, BOOKS b WHERE s.Isbn = ? AND b.Isbn= ? AND b.IdCustomer = ?', [r.item(i).Isbn, r.item(i).Isbn, id], function(tx, results) {
							for(var j = 0; j < results.rows.length; j++) {
								arr.push(results.rows.item(j));
							}
							if(arr.length > 0) deferred.resolve(arr);
